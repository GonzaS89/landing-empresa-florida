...
export const Paradas = ({ nombre, index, length, origen, destino }) => {
  ...
  return (
    ...
    <>
      ...
      >
        <p className={`${esLocalidad(nombre) ? '' : 'text-xs'} ${nombreOrigen || nombreDestino ? 'bg-zinc-800' : ''} text-sm uppercase font-jockey z-50 text-center`}>{esLocalidad(nombre) ? nombre : index === 0 ? `desde ${nombre}` : index === (length - 1) ? `hasta ${nombre}` : `por ${nombre}`}</p>
        ...
        transition={{ duration: 2 , repeat: Infinity, delay: nombreDestino ? 3 : 2 }}
