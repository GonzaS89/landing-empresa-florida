...
export const Consulta = forwardRef((props, ref) => {
  ...
  return (
    ...
    className={`${returnBg(props.titulo)} sm:h-[450px] lg:h-auto flex flex-col justify-center lg:justify-evenly items-start py-8 px-8 pr-16 gap-8  md:gap-10 lg:gap-4 xl:gap-2`}>
        ...
        <Link to={props.titulo === 'Abonos' ? '/cotizadordeabonos' : '/consultadehorarios'}>
        <button className={`${returnButtonStyle(props.titulo)} min-w-44 p-2 md:p-4 uppercase font-bold md:text-lg xl:text-xl font-secundaria tracking-wider hover:scale-110 transition-all duration-300 ease-in-out`}>
          {props.boton}
          </button>
        </Link>
